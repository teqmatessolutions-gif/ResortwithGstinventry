  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <KpiCard title="Total Revenue" value={`Γé╣${totalRevenue.toLocaleString()}`} color="bg-gradient-to-r from-purple-500 to-purple-700" icon={<i className="fas fa-rupee-sign"></i>} />
      </div>
              ...
              <motion.div key={pkg.id} whileHover={{ y: -5 }} className="bg-gray-50 rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-all duration-300 flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="flex justify-between items-center mt-auto pt-4 border-t border-gray-200">
                    <p className="text-green-600 font-bold text-2xl">Γé╣{pkg.price.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
        ...
        {editingPackage && (
          <Card title={`Edit Package: ${editingPackage.title}`}>
            <form onSubmit={handleEditSubmit} className="space-y-6">
              ...
              <input type="number" name="price" placeholder="Price (Γé╣)" value={editForm.price} onChange={handleEditChange} className="w-full p-3 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-all" required />
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="w-1/2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-lg shadow-md transition-transform transform hover:-translate-y-1">Update Package Γ£Å∩╕Å</button>
              </div>
            </form>
          </Card>
        )}
        ...
        {!editingPackage && (
          ...
          <form onSubmit={handleCreateSubmit} className="space-y-6">
            ...
            <input type="number" name="price" placeholder="Price (Γé╣)" value={createForm.price} onChange={handleCreateChange} className="w-full p-3 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-all" required />
            ...
            <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-lg shadow-md transition-transform transform hover:-translate-y-1">Create Package ≡ƒÜÇ</button>
          </form>
        </Card>
        )}
        ...
        <Card title={editingBooking ? "Edit Booking" : "Book a Package"}>
          <form onSubmit={handleBookingSubmit} className="space-y-6">
            <select name="package_id" value={bookingForm.package_id} onChange={handleBookingChange} className="w-full p-3 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-all" required>
              ...
              {packages.map(p => (<option key={p.id} value={p.id}>{p.title} - Γé╣{p.price}</option>))}
            </select>
                      ...
                      {availableRooms.length > 0 ? (
                        availableRooms.map(room => (
                          ...
                          >
                            ...
                            <p className={`text-xs ${bookingForm.room_ids.includes(room.id) ? 'text-indigo-200' : 'text-gray-500'}`}>Γé╣{room.price}/night</p>
                          </div>
                        ))
                      ) : (
            ...
            <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-lg shadow-md transition-transform transform hover:-translate-y-1">
              {editingBooking ? "Update Booking Γ£Å∩╕Å" : "Book Package Γ£à"}
            </button>
          </form>
        </Card>
      </div>
