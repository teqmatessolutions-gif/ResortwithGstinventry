  ...
  return (
    <DashboardLayout>
      <div className="p-6 space-y-8">
        <h2 className="text-3xl font-extrabold text-indigo-700 mb-4">≡ƒì╜ Food Orders Dashboard</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-5 flex flex-col items-center">
            ...
            <p className="text-2xl font-bold text-green-600">Γé╣{totalRevenue}</p>
            <div className="w-full h-20 mt-2">
    ...
    rooms.map((room) => (
      <option key={room.id} value={room.id}>
        Room {room.number || room.room_number || room.id}
          ...
          <div className="text-lg font-semibold mt-2">Total: Γé╣{amount}</div>
                 <button
          ...
          {filteredOrders.map((order) => {
            ...
            return (
              ...
              >
                ...
                <div className="font-semibold text-lg mb-2">Γé╣{order.amount}</div>
                <ul className="list-disc ml-5 text-sm mb-2">
                  {order.items.map((item, i) => (
                    <li key={i}>
                      {item.food_item_name} ├ù {item.quantity}
                    </li>
                  ))}
                </ul>
