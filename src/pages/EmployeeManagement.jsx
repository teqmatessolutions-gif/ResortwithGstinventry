        ...
        <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
                <button onClick={() => setShowCreateForm(!showCreateForm)} className="font-semibold text-indigo-600">
                    {showCreateForm ? 'Γû╝ Hide Form' : 'Γû╢ Create Leave Entry'}
                </button>
                {showCreateForm && (
            ...
            {(() => {
              ...
              return (
                <div className={`p-2 rounded-md text-center text-sm font-semibold ${hasOpenClockIn ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                  Status: {hasOpenClockIn ? '≡ƒƒó Clocked In' : 'ΓÜ¬ Not Clocked In'}
                </div>
              );
            })()}
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <button className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                              {selectedDay === day.date ? 'Γû▓ Hide Details' : 'Γû╝ Show Details'}
                            </button>
                          </td>
                        </tr>
          ...
          <tbody>
            {filteredEmployees.map((emp, i) => (
              <tr key={emp.id} className="hover:bg-gray-50">
                ...
                <td className="p-1 sm:p-2 border text-right text-xs sm:text-sm">{emp.salary ? `Γé╣${emp.salary}` : 'N/A'}</td>
