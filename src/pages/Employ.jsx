...
const Employee = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Γ£à Rebuild the FormData object explicitly
    ...
    // Γ£à Add phone and image fields if they exist
    if (form.phone) {
  ...
  const handleEdit = (emp) => {
    ...
    setForm({
      ...
      email: emp.email, // Γ£à Use email from the employee data
      phone: emp.phone, // Γ£à Use phone from the employee data
      password: "", // Γ£à Keep password as empty for security
      image: null,
    });
              ...
              <div className="absolute inset-0 bg-white bg-opacity-95 flex items-center justify-center rounded-xl p-2 shadow-lg">
                <ResponsiveContainer width="100%" height={50}>
                  <LineChart data={kpi.trend.map((v, i) => ({ day: i + 1, value: v }))}>
                    ...
                    <Tooltip contentStyle={{ fontSize: 12 }} formatter={(val) => [`Γé╣${val}`, "Salary"]} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <tbody>
            {filteredEmployees.map((emp, i) => (
              <tr key={emp.id} className="hover:bg-gray-50">
                ...
                <td className="p-2 border text-right">Γé╣{emp.salary}</td>
